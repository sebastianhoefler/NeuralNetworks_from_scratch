...
class Conv():
    ...
    def backward(self, error_tensor):
        ...
        #     output_back_shape = np.zeros((error_tensor.shape[0], self.input_tensor.shape[1], self.input_tensor.shape[2]))

        # elif self.input_tensor.ndim == 4:
        #     output_back_shape = np.zeros((error_tensor.shape[0], self.input_tensor.shape[1], self.input_tensor.shape[2], self.input_tensor.shape[3]))

        output_back_shape = np.zeros_like(self.input_tensor)

        # rearrange the filters. First channels of every filter go into one filter. 
        backward_filter = np.swapaxes(self.weights,0,1)

        # This whole loop is fkd. Not sure whats happening and why it is failing?!?! Needs redo!
        # Perform the convolution
        # for i in range(error_tensor.shape[0]): #line is correct
        #     for j in range(backward_filter.shape[0]): # line is correct
        #         # Reverse the channel kernel dimension
        #         kernel = backward_filter[j,::-1] # flipping doesn'T change anything! Why?
        #         conv_out = scipy.signal.convolve(error_tensor[i], kernel, mode='same')
        #         sum_out = np.sum(conv_out, axis=0)
        #         output_back_shape[i,j] += sum_out # Sum over all kernels
        #print('error_tensor.shape', error_tensor.shape)
        #print('backward_filter.shape', backward_filter.shape)
        ...
        # gradient w.r.t bias. For 2D sum of error tensor along axis
    ...
    def initialize(self, weights_initializer, bias_initializer):
        ...
        # The arguemnt is something like weights_initilizer=He and then we call He.initialize(...)
